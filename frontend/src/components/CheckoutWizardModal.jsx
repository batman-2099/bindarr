...
export const CheckoutSelectionModal = ({ cards, initialAllocations, busy, error, onCancel, onConfirm }) => {
  const { t } = useT();
  const [quantities, setQuantities] = useState(() => Object.fromEntries(
    initialAllocations.map(allocation => [allocation.entry_id, allocation.quantity])
  ));
  const cancel = () => { if (!busy) onCancel(); };
  useBackGuard(true, cancel);

  const valid = cards.every(card => card.choices.every(choice => {
    const quantity = quantities[choice.entry_id] ?? 0;
    return Number.isInteger(quantity) && quantity >= 0 && quantity <= choice.available_qty;
  }) && card.choices.reduce((sum, choice) => sum + Number(quantities[choice.entry_id] || 0), 0) === card.quantity);

  const confirm = () => {
    if (busy || !valid) return;
    onConfirm(cards.flatMap(card => card.choices.flatMap(choice => {
      const quantity = quantities[choice.entry_id] || 0;
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="checkout-selection-title"
        aria-describedby="checkout-selection-hint"
        aria-busy={busy}
        style={{ maxWidth: '640px', width: '100%', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}
        onClick={event => event.stopPropagation()}
      >
        <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border-glass)', display: 'flex', justifyContent: 'space-between', gap: '1rem' }}>
          <div>
            <h2 id="checkout-selection-title" style={{ margin: '0 0 0.25rem', fontSize: '1.35rem', color: 'var(--text-strong)' }}>{t('wizard.choose.title')}</h2>
            <p id="checkout-selection-hint" style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-secondary)' }}>{t('wizard.choose.subtitle')}</p>
          </div>
          <button className="btn btn-secondary btn-icon-only" onClick={cancel} disabled={busy} aria-label={t('common.cancel')} autoFocus><X size={16} /></button>
        </div>
        <div style={{ padding: '1.25rem 1.5rem', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {cards.map(card => {
            const selected = card.choices.reduce((sum, choice) => sum + Number(quantities[choice.entry_id] || 0), 0);
            const available = card.choices.reduce((sum, choice) => sum + choice.available_qty, 0);
            return (
              <fieldset key={card.card_id} disabled={busy} style={{ border: '1px solid var(--border-glass)', borderRadius: 'var(--radius-md)', padding: '0.85rem', minWidth: 0 }}>
                <legend style={{ fontWeight: 700, color: 'var(--text-strong)' }}>{card.name}</legend>
                <p aria-live="polite" style={{ margin: '0 0 0.65rem', color: selected === card.quantity ? 'var(--text-secondary)' : 'var(--accent-red)', fontSize: '0.85rem' }}>{t('wizard.choose.quantity', { selected, needed: card.quantity })}</p>
                {available < card.quantity && <p style={{ color: 'var(--accent-red)', fontSize: '0.85rem' }}>{t('wizard.notEnoughCopies')}</p>}
                {card.choices.map(choice => (
                  <label key={choice.entry_id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', padding: '0.4rem 0' }}>
                    <span style={{ fontSize: '0.85rem' }}>
                      <span style={{ display: 'block', color: 'var(--text-strong)' }}>
                        {choice.location_name || t('bulk.unassignedPile')}
                        {choice.compartment_display ? ` · ${choice.compartment_display}` : ''}
                        {slotOf(choice.position) ? ` · ${t('wizard.slot', { slot: slotOf(choice.position) })}` : ''}
                      </span>
                      <span style={{ color: 'var(--text-secondary)' }}>{t('wizard.choose.available', { count: choice.available_qty })}</span>
                    </span>
                    <input
                      type="number"
                      className="input-control"
                      min="0"
                      max={Math.min(choice.available_qty, card.quantity)}
                      step="1"
                      value={quantities[choice.entry_id] ?? 0}
                      disabled={choice.available_qty === 0}
                      onChange={event => setQuantities(previous => ({ ...previous, [choice.entry_id]: event.target.value === '' ? '' : Number(event.target.value) }))}
          ...
          {error && <p role="alert" style={{ margin: 0, color: 'var(--accent-red)' }}>{error}</p>}
        </div>
        <div style={{ padding: '1rem 1.5rem', borderTop: '1px solid var(--border-glass)', display: 'flex', justifyContent: 'flex-end', gap: '0.75rem' }}>
          <button className="btn btn-secondary" onClick={cancel} disabled={busy}>{t('common.cancel')}</button>
          <button className="btn btn-primary" onClick={confirm} disabled={busy || !valid}>{t('wizard.choose.confirm')}</button>
        </div>
      </div>
    </div>
  );
